		...
		if (opt.success != null && opt.success != undefined) {
			...
			opt.success = function(data) {
				...
				if (data.message != null && data.message != undefined && data.message != "") {
					window._MRHLoader.slowDestroy(500,data.message);
					success(data);
				} else {
					...
					window._MRHLoader.load(str);
					success(data);
					window._MRHLoader.slowDestroy(1000);
				}
...
window._formOption = function(form, callback) {
	...
	var options = {
		...
		error : function(){
			window._MRHLoader.slowDestroy(3000,"服务器出错，请稍后重试","wrong");
		}
	}
	return options;
}
...
window._MRHLoader = {
		t:null,
		$message : null,
		load : function(text,flag,returnUrl){
			var strHtml = "<div class='body_message text-center'><span>"+text
						+ "</span></div>";
			window._MRHLoader.$message = $(strHtml);
			if(flag){
				window._MRHLoader.$message.addClass("wrong");
			}
			if(returnUrl){
				window._MRHLoader.$message.append($('<a href='+window._BasePath +'"/i/index.jspx"> 返回首页</a>'));
			}
			$("body").append(window._MRHLoader.$message);
		},
		destroy : function(){
			$("body .body_message").remove();
		},
		slowDestroy :function(delay,text,flag,returnUrl){
			var $htmlMsg = $("body").find(".body_message");
			if($htmlMsg != undefined){window.clearInterval(window._MRHLoader.t);$htmlMsg.remove();}
			var opa = 0.88;
			if(text != undefined){
				window._MRHLoader.load(text,flag,returnUrl);
			}else{
				window._MRHLoader.load("加载完毕");
			}
			var message = window._MRHLoader.$message[0];
			if(delay != undefined){
